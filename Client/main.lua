...
local function LoadESXVersion()
        ...
        options = {
            {
                ...
                event = 'ProjextX-OrangeFarm:ProcessOrange',
                icon = 'fa-solid fa-filter',
                label = Config.Text["ProcessingLabel"],
            },
            {
                name = 'SellingRawOranges',
                event = 'ProjextX-OrangeFarm:SellRawOrange',
    ...
    exports.ox_target:addBoxZone({
        ...
        options = {
            {
                name = 'selljuices',
                event = 'ProjextX-OrangeFarm:SellingJuice',
    ...
    RegisterNetEvent('ProjextX-OrangeFarm:PickingOranges', function()
        ...
        if success then
            ...
            TriggerServerEvent('ProjextX-OrangeFarm:GratherOrange')
        else
    ...
    --Processing Event
    RegisterNetEvent('ProjextX-OrangeFarm:ProcessOrange', function()
        local Ped = PlayerPedId()
        FreezeEntityPosition(Ped, true)
        ...
        then
            TriggerServerEvent("ProjextX-OrangeFarm:TradingToOrangeJuice")
        else
    ...
    RegisterNetEvent('ProjextX-OrangeFarm:SellRawOrange', function()
        local Ped = PlayerPedId()
        FreezeEntityPosition(Ped, true)
        if lib.progressCircle({
            duration = 4000,
        ...
        then
            TriggerServerEvent("ProjextX-OrangeFarm:SellingRawOrange")
        else
            ESX.ShowNotification(Config.Text["CancelledProcessing"], "error")
        end
        FreezeEntityPosition(Ped, false)
    ...
    RegisterNetEvent('ProjextX-OrangeFarm:SellingJuice', function()
        local Ped = PlayerPedId()
        FreezeEntityPosition(Ped, true)
        if lib.progressCircle({
            duration = 4000,
        ...
        then
            TriggerServerEvent("ProjextX-OrangeFarm:SellingJuices")
        else
            ESX.ShowNotification(Config.Text["CancelledProcessing"], "error")
        end
        FreezeEntityPosition(Ped, false)
...
local function LoadQBVersion()
    ...
    elseif Config.Target == "QB" then
        CreateThread(function()
            for _, v in pairs(OrangeTrees) do
                exports['qb-target']:AddBoxZone(v.Name, vector3(v.coords.x, v.coords.y, v.coords.z), v.size.x, v.size.y, {
                    name = v.Name,
                    heading = v.heading,
                    debugPoly = false,
                    minZ = v.coords.z - 2,
        ...
        exports.ox_target:addBoxZone({
            exports['qb-target']:AddBoxZone(v.Name, vector3(Config.OXTarget.processingCoords.x, Config.OXTarget.processingCoords.y, Config.OXTarget.processingCoords.z), 1, 1, {
                name = "Processed",
        ...
        exports['qb-target']:AddBoxZone(v.Name, vector3(Config.OXTarget.sellingCoords.x, Config.OXTarget.sellingCoords.y, Config.OXTarget.sellingCoords.z), 1, 1, {
            name = "Seller",
